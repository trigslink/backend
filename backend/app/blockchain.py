...
# Register function
def register_on_chain(service_name: str, https_uri: str, price: int, duration: int) -> str:
    nonce = w3.eth.get_transaction_count(wallet_address)

    # Here, only price is used since mintLicense only accepts one argument.
    txn = contract.functions.mintLicense(price).build_transaction({
        'chainId': CHAIN_ID,
        'gas': 250000,
        'gasPrice': w3.eth.gas_price,
        'nonce': nonce,
        'from': wallet_address,
        'value': price  # required since function is payable
    })
